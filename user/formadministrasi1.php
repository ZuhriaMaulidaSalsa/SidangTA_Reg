...
$query_status = "SELECT `status_form_administrasi` FROM `multiuser` WHERE nim = '$nim';";
$hasil_status = mysqli_query($koneksi, $query_status);

if ($hasil_status) {
    $row_status = mysqli_fetch_assoc($hasil_status);
    $status = $row_status['status_form_administrasi'];
} else {
    ...
    <section>
        ...
        <div class="dashboard" style="margin-top: -15px; box-sizing: border-box; font-family: 'Poppins', sans-serif;">
            <div class="dash-content">
                <div class="overview">
                    <?php if ($status == 'SEDANG DITINJAU') : ?>
                        <!-- Box 1: Tampilkan jika status di tinjau -->
                        <div class="status-box-ditinjau" style="background-color: rgba(251, 140, 1, 0.2); color: #721c24; border: 1.5px solid rgb(251, 140, 1);">
                            <!-- Isi box 1 -->
                            <br><br>
                    ...
                    <?php elseif ($status == 'DISETUJUI') : ?>
                        <!-- Box 2: Tampilkan jika status disetujui -->
                        <div class="status-box-disetujui" style="background-color: rgba(173, 216, 230, 0.3); color: #007bff; border: 1px solid #007bff;">
                            ...
                            Jadwal Sidang Akan Ditampilkan di Profil

                        </div>
                    <?php else : ?>
                        <!-- Tampilkan pesan default jika status tidak sesuai -->
                        <div class="status-box" style="background-color: #eee; color: #333; border: 1px solid #ccc;">
                            Status tidak valid.
                        </div>
                    <?php endif; ?>
                </div>
